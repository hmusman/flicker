      ...
      <div style="width: 100%;" id="sellProducts">
          @if($products->count()>0)
            @foreach($products as $row)
               @include('partials.sell_products_list')
            @endforeach
          @endif
      </div>
    </div>
...
<script>
  ...
  function FetchBrandProducts(brands,options)
  {
    $.ajax({
        url:"{{ route('BrandsSellProducts') }}",
        type:"get",
        data:{brands:brands},
  ...
  function FetchPriceProducts(from,to,options)
  {
    $.ajax({
        url:"{{ route('PriceSellProducts') }}",
        type:"get",
  ...
  function FetchCityProducts(cities,options)
  {
    $.ajax({
        url:"{{ route('CitySellProducts') }}",
        type:"get",
        data:{cities:cities},
        success:function(data){
          setTimeout(function () {
  ...
  function FetchStatusProducts(statuses,options)
  {
    $.ajax({
        url:"{{ route('StatusSellProducts') }}",
        type:"get",
  ...
  function FetchBrandsPriceCityStatusProducts(brands,from,to,cities,statuses,options)
  {
    $.ajax({
        url:"{{ route('BrandsPriceCityStatusSellProducts') }}",
        type:"get",
  ...
  $('.brands_check').change(function(){
      ...
      if($('.brands_check').is(":checked"))
      {
          var brands = $('.brands_check:checked').map(function(){ return $(this).val();}).get();
          FetchBrandProducts(brands,options);
      }
  });

  $('.city_check').change(function(){

      if($('.city_check').is(":checked"))
      {
          var cities = $('.city_check:checked').map(function(){ return $(this).val();}).get();
          FetchCityProducts(cities,options);
      }
  });

   $('.device_status_check').change(function(){

      if($('.device_status_check').is(":checked") && $('.device_status_check').is(":checked") && $('.device_status_check').is(":checked") && $('#from_price').val() !='' && $('#to_price').val() !='')
      {
          var brands = $('.brands_check:checked').map(function(){ return $(this).val();}).get();
          ...
          FetchBrandsPriceCityStatusProducts(brands,from,to,cities,statuses,options);
      }
      // else if($('.device_status_check').is(":checked"))
      // {
      //     var statuses = $('.device_status_check:checked').map(function(){ return $(this).val();}).get();
      //     FetchStatusProducts(statuses,options);
      // }
  });
  ...
  $('#go_price').click(function(){
      ...
      if(from!='' && to!='')
      {
        ...
        { 
          var brands = $('.brands_check:checked').map(function(){ return $(this).val();}).get();
           $.ajax({
              url:"{{ route('BrandsPriceSellProducts') }}",
              type:"get",
        ...
        {
          FetchPriceProducts(from,to,options);
        }
      }
